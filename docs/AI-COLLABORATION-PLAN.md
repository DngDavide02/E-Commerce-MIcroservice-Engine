...
| 2 | Base Services (CRUD) | 50% | In progress — Product Service CRUD done by AI (no business logic to reserve there). Inventory Service: AI added basic CRUD only (Entity, DTOs, Repository, Service, Controller for creating/reading stock records) — reserve/release quantity logic and the OrderCreated event consumer are intentionally NOT implemented; that's still the user's part |
