...
- [ ] **Phase 2 — Inventory Service**: basic CRUD done by AI (`InventoryItem` entity — `productId` + `quantity`, Repository, Service, Controller at `/api/inventory` for create/list/find-by-product). Reserve/release logic and the `OrderCreated` → `StockReserved`/`StockRejected` consumer are **not implemented yet on purpose** — that's the hand-written part per the AI collaboration plan. 3 unit tests passing; Testcontainers IT written but not run here (needs Docker).
