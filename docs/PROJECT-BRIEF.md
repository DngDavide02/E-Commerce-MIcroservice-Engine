...
- [ ] **Phase 2 — Inventory Service**: basic CRUD done by AI (`InventoryItem` entity — `productId` + `quantity`, Repository, Service, Controller at `/api/inventory` for create/list/find-by-product). `reserve(productId, quantity)` **hand-written by the user** — decrements stock, throws `InsufficientStockException` when short. `release()` and the `OrderCreated`/`ReleaseStock` event wiring still to come. 5 unit tests passing (verified by AI after merge). Testcontainers IT written but not run here (needs Docker).
- [ ] **Phase 3 — Payment Service**: basic CRUD done by AI (`PaymentTransaction` — `orderId` + `amount` + `status` enum PENDING/COMPLETED/FAILED, Repository, Service, Controller at `/api/payments` for create/list/find-by-order, always starting PENDING). The authorize/reject decision logic (the participant behaviour triggered by `StockReserved`, publishing `PaymentCompleted`/`PaymentFailed`) is **not implemented on purpose** — same boundary as Inventory's reserve/release, left for hand-written work. 3 unit tests passing; Testcontainers IT written but not run here (needs Docker).
